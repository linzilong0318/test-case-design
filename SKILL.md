...
description: 这项技能用于后端驱动的全流程测试用例设计：从 PDF 需求文档下载与解析、需求整理与确认、测试用例生成与 API 上传、评审文档生成与上传，到最终总结。涵盖功能测试、接口测试、AI Agent 测试、兼容性测试、UI 测试、联动测试、路由测试及多平台专项测试（移动端/小程序/H5/桌面/PC Web）。仅专注于编写测试用例，不涉及测试计划、测试策略或自动化脚本。
---
...
- **阶段一（需求获取）**：用户请求中包含 PDF 下载链接 + "理解并整理需求"/"下载"等关键词
...
从用户请求中提取：
- `pdfUrl`：PDF 需求文档的下载链接（通常在 "下载" 后面或 HTTP 链接格式）
...
# 下载 PDF（文件名从 URL 中提取或使用时间戳）
curl -L -o "/tmp/test-case-design/{sessionId}/requirements.pdf" "{pdfUrl}"
```

> **注意**：下载前检查临时目录是否已存在，如存在则先清空。

### 1.3 解析 PDF 并提取需求

1. 调用 PDF 解析 skill 阅读下载的 PDF 文档
2. 基于 PDF 内容，**判断系统类型**以便后续阶段加载正确的测试能力文件：
...
调用已有的 Nacos skill，获取注册在 Nacos 上的后端服务 IP 地址。
...
基于确认后的需求和生成的用例，按 `references/templates/clarification-checklist.md` 模板生成 Markdown 文档。
...
将生成的 MD 内容写入临时文件：
```bash
echo "{md_content}" > "/tmp/test-case-design/{sessionId}/待澄清需求清单_{batchNo}.md"
```

### 3.2 生成「测试用例评审报告」

对生成的用例进行自我评审，按 `references/templates/review-report.md` 模板生成 Markdown 文档。
...
将生成的 MD 内容写入临时文件：
```bash
echo "{md_content}" > "/tmp/test-case-design/{sessionId}/测试用例评审报告_{batchNo}.md"
```
...
> **目标**：确保本地不残留 PDF 和 MD 文件，保持本地环境干净。
