...
当测试用例数量较多（>30条）时，使用本模式高效生成。避免在 Python 脚本中使用复杂的数据结构嵌套，改用函数式快速录入。
...
out_path = f"/opt/data/tmp/test-case-design/{SESSION_ID}/cases_payload.json"
...
4. **JSON 文件 → curl 上传**：先生成 .json 文件，再用 `@file` 方式 curl 上传
...
| 逐个打印用例到 stdout | 无法用好 json.dump | 写 .json 文件 + curl -d @file |
| 用 for 循环遍历变量生成 | 逻辑复杂，调试困难 | 显式逐条调用 tc() |

### 上传方式

```bash
curl -X POST "{ip}:{port}/{service_name}/api/v1/testcase/save" \
  -H "Content-Type: application/json" \
  -d @/opt/data/tmp/test-case-design/{sessionId}/cases_payload.json
```
